...
from swift.common.middleware import server_side_copy
...
class TestServerSideCopyMiddleware(unittest.TestCase):
    def setUp(self):
        self.app = FakeSwift()
        self.ssc = server_side_copy.filter_factory({
    ...
    def test_POST_as_COPY_static_large_object(self):
        ...
        method, path, req_headers = calls[1]
        self.assertEqual('PUT', method)
        # Make sure the sink request (PUT) has the header
        self.assertTrue('X-Static-Large-Object' in req_headers)

    def test_PUT_log_info(self):
        self.app.register('GET', '/v1/a/some/where', swob.HTTPOk, {}, 'passed')
        self.app.register('PUT', '/v1/a/c/o', swob.HTTPCreated, {})
        req = Request.blank('/v1/a/c/o', method='PUT',
                            headers={'X-Copy-From': 'some/where',
                                     'Content-Length': 0})
        status, headers, body = self.call_ssc(req)
        self.assertEqual(status, '201 Created')
        self.assertEqual(
        ...
        self.app.register('GET', '/v1/a/else/where', swob.HTTPOk, {}, 'passed')
        self.app.register('PUT', '/v1/a/c/o', swob.HTTPCreated, {})
        req = Request.blank('/v1/a/c/o', method='POST',
                            headers={'X-Copy-From': 'else/where'})
        self.assertEqual(status, '201 Created')
    ...
    def test_static_large_object(self):
        self.app.register('GET', '/v1/a/c/o', swob.HTTPOk,
                          {'X-Static-Large-Object': 'True'}, 'passed')
        self.app.register('PUT', '/v1/a/c/o2?multipart-manifest=get',
                          swob.HTTPCreated, {})
        ...
        method, path, req_headers = calls[1]
        self.assertEqual('PUT', method)
        self.assertEqual('/v1/a/c/o2?multipart-manifest=get', path)
        self.assertEqual(req_headers['X-Static-Large-Object'],
                         'True')
    ...
    def test_copy_source_larger_than_max_file_size(self):
        ...
        req = Request.blank('/v1/a/c/o', environ={'REQUEST_METHOD': 'PUT'},
                            headers={'Content-Length': '0',
                                     'X-Copy-From': '/c/o'})
        with mock.patch('swift.common.middleware.server_side_copy.'
                        'MAX_FILE_SIZE', 1):
            status, headers, body = self.call_ssc(req)
...
class TestServerSideCopyConfiguration(unittest.TestCase):
    ...
    def test_reading_proxy_conf_when_no_middleware_conf_present(self):
        ...
        [pipeline:main]
        pipeline = catch_errors server_side_copy ye-olde-proxy-server

        [filter:server_side_copy]
        use = egg:swift#server_side_copy
        ...
        ssc = server_side_copy.filter_factory({
